...
import { createOpenClawTools } from "../agents/blockclaw-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-blockclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-blockclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-blockclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-blockclaw-thread-id")?.trim() || undefined;
