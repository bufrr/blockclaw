...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "blockclaw-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.blockclaw-live-tool-probe.${nonceA}.txt`);
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "blockclaw-live-"));
  const tempConfigPath = path.join(tempDir, "blockclaw.json");
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "BlockClaw live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.blockclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
