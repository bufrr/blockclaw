...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("blockclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "blockclaw-sandbox-browser-global" },
      agentBrowser: { network: "blockclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("blockclaw-sandbox-browser-agent");
  });
