...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.blockclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.blockclawCanvasA2UIAction &&
        typeof window.blockclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.blockclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("blockclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android BlockClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.blockclawSendUserAction === "function"
        ? window.blockclawSendUserAction
        : undefined;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>BlockClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
