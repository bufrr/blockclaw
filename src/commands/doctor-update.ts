...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update BlockClaw from git before running doctor?",
      initialValue: true,
    });
    if (!shouldUpdate) {
      return { updated: false };
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("blockclaw update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
