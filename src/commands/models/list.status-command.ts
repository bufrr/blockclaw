...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("blockclaw models auth setup-token")}\` or \`${formatCliCommand("blockclaw configure")}\`.`
          : `Run \`${formatCliCommand("blockclaw configure")}\` or set an API key env var.`;
