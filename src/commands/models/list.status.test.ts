...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenClawAgentDir: vi.fn().mockReturnValue("/tmp/blockclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/blockclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/blockclaw-agent/auth-profiles.json"),
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/blockclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/blockclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/blockclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/blockclaw-agent-custom");
