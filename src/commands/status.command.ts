...
export async function statusCommand(
  ...
  runtime.log(theme.heading("BlockClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`blockclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("blockclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("blockclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("blockclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("blockclaw security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("blockclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("blockclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("blockclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("blockclaw gateway probe")}`);
  }
}
