...
describe("security audit", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "blockclaw-security-audit-"));
  ...
  it("warns for risky safeBinTrustedDirs entries", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/blockclaw-safe-bins"];
    const cfg: OpenClawConfig = {
      tools: {
        exec: {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "blockclaw.json");
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("blockclaw-sbx-browser-old\nblockclaw-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "blockclaw-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "blockclaw-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("blockclaw-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("blockclaw-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "blockclaw-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "blockclaw-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-blockclaw.json");
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.blockclaw/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.blockclaw",
      configPath: "/Users/test/Dropbox/.blockclaw/blockclaw.json",
    });
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "blockclaw.json"),
      });
