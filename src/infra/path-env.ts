...
function candidateBinDirs(opts: EnsureOpenClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `blockclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "blockclaw");
  ...
  if (allowProjectLocalBin) {
    ...
    if (isExecutable(path.join(localBinDir, "blockclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `blockclaw` CLI can run
