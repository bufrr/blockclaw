...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "blockclaw-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".blockclaw", "agents", "main", "sessions"), {
      recursive: true,
    });
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(base, ".blockclaw");
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".blockclaw", "agents", "main", "agent");
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    const stateDir = path.join(home, ".blockclaw");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.writeFile(
      path.join(stateDir, "blockclaw.json"),
