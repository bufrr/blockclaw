...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "blockclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "blockclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "blockclaw-gw");
  ...
  records.push(`_blockclaw-gw._tcp IN PTR ${instanceLabel}._blockclaw-gw._tcp`);
  records.push(`${instanceLabel}._blockclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._blockclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; blockclaw-content-hash: ${contentHash}\n${contentBody}`;
}
