...
describe("infra parsing", () => {
  ...
  describe("isMainModule", () => {
    ...
    it("returns true for dist/entry.js when launched via blockclaw.mjs wrapper", () => {
      expect(
        isMainModule({
          currentFile: "/repo/dist/entry.js",
          argv: ["node", "/repo/blockclaw.mjs"],
          cwd: "/repo",
          env: {},
          wrapperEntryPairs: [{ wrapperBasename: "blockclaw.mjs", entryBasename: "entry.js" }],
        }),
      ).toBe(true);
    });
    ...
    it("returns false when running under PM2 but this module is imported", () => {
      expect(
        isMainModule({
          currentFile: "/repo/node_modules/blockclaw/dist/index.js",
