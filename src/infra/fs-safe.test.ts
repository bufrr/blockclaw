...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("blockclaw-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("blockclaw-fs-safe-root-");
    const outside = await tempDirs.make("blockclaw-fs-safe-outside-");
