...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("blockclaw node install"),
    formatCliCommand("blockclaw node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("blockclaw node install --force")}`);
    }
    return;
  }
