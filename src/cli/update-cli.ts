...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update BlockClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["blockclaw update", "Update a source checkout (git)"],
        ["blockclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["blockclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["blockclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["blockclaw update --dry-run", "Preview actions without changing anything"],
        ["blockclaw update --no-restart", "Update without restarting the service"],
        ["blockclaw update --json", "Output result as JSON"],
        ["blockclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["blockclaw update wizard", "Interactive update wizard"],
        ["blockclaw --update", "Shorthand for blockclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run blockclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openclaw.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["blockclaw update status", "Show channel + version status."],
          ["blockclaw update status --json", "JSON output."],
          ["blockclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
