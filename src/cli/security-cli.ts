...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["blockclaw security audit", "Run a local security audit."],
          ["blockclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["blockclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["blockclaw security audit --json", "Output machine-readable JSON."],
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("BlockClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("blockclaw security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("blockclaw security audit --fix")}`));
        if (!fixResult) {
