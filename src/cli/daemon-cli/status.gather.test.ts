...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENCLAW_STATE_DIR: "/tmp/blockclaw-daemon",
    OPENCLAW_CONFIG_PATH: "/tmp/blockclaw-daemon/blockclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENCLAW_STATE_DIR ?? "/tmp/blockclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENCLAW_CONFIG_PATH ?? `${stateDir}/blockclaw.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/blockclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENCLAW_STATE_DIR = "/tmp/blockclaw-cli";
    process.env.OPENCLAW_CONFIG_PATH = "/tmp/blockclaw-cli/blockclaw.json";
