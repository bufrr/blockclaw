...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("blockclaw gateway install", env)}`);
    if (fileLog) {
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("blockclaw gateway install", env),
    formatCliCommand("blockclaw gateway", env),
  ];
