...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall BlockClaw and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “BlockClaw Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("blockclaw browser extension install")}"`,
