...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "blockclaw", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "blockclaw", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "blockclaw", "status"]);
  });
