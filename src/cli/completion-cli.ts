...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# BlockClaw Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(blockclaw completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# BlockClaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(blockclaw completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "blockclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c blockclaw -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'blockclaw gateway' and not 'blockclaw other gateway', we need to check the sequence.
