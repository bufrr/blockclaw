...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "blockclaw.sandbox=1");
  args.push("--label", `blockclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `blockclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `blockclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "blockclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`blockclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`blockclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("blockclaw sandbox recreate --all");
}
