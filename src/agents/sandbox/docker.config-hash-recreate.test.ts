  ...
  return {
    ...
    spawn: (command: string, args: string[]) => {
      ...
      if (command !== "docker") {
        ...
        args[0] === "inspect" &&
        args[1] === "-f" &&
        args[2]?.includes('index .Config.Labels "blockclaw.configHash"')
      ) {
...
function createSandboxConfig(dns: string[], binds?: string[]): SandboxConfig {
  return {
    ...
    browser: {
      enabled: false,
      image: "blockclaw-browser:test",
      containerPrefix: "oc-browser-",
      network: "blockclaw-sandbox-browser",
      cdpPort: 9222,
      vncPort: 5900,
      noVncPort: 6080,
      headless: true,
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall?.args).toContain(`blockclaw.configHash=${newHash}`);
  ...
  it("applies custom binds after workspace mounts so overlapping binds can override", async () => {
    ...
    expect(createCall).toBeDefined();
    expect(createCall?.args).toContain(`blockclaw.configHash=${expectedHash}`);
