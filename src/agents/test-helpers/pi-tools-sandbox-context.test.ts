...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("blockclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("blockclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("blockclaw-sbx-");
