...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENCLAW_HOME for default workspace resolution", () => {
    ...
    expect(dir).toBe(path.join(path.resolve("/srv/blockclaw-home"), ".blockclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".blockclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("blockclaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "blockclaw-workspace-hardlink-"));
    try {
