...
const RELAY_AUTH_HEADER = "x-blockclaw-relay-token";
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/BlockClaw-Extension-Relay",
            revision: "0",
            userAgent: "BlockClaw-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "BlockClaw/extension-relay",
