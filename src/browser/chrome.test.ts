...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "blockclaw-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".blockclaw-profile-decorated"),
      "utf-8",
    );
...
describe("browser chrome helpers", () => {
  ...
  it("stopBlockClawChrome no-ops when process is already killed", async () => {
  ...
  it("stopBlockClawChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopBlockClawChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
