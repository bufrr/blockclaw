...
export async function launchOpenClawChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🦞 blockclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`blockclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`blockclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 blockclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
