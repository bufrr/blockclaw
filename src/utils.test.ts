...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("blockclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("blockclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("blockclaw-lid-a-", (first) => {
      withTempDirSync("blockclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.blockclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "blockclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENCLAW_HOME over HOME", () => {
    vi.stubEnv("OPENCLAW_HOME", "/srv/blockclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/blockclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENCLAW_HOME prefix when OPENCLAW_HOME is set", () => {
    ...
    expect(
      shortenHomePath(`${path.resolve("/srv/blockclaw-home")}/.blockclaw/blockclaw.json`),
    ).toBe("$OPENCLAW_HOME/.blockclaw/blockclaw.json");
...
describe("shortenHomeInString", () => {
  it("uses $OPENCLAW_HOME replacement when OPENCLAW_HOME is set", () => {
    ...
    expect(
      shortenHomeInString(
        `config: ${path.resolve("/srv/blockclaw-home")}/.blockclaw/blockclaw.json`,
      ),
    ).toBe("config: $OPENCLAW_HOME/.blockclaw/blockclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/blockclaw")).toBe(path.resolve(os.homedir(), "blockclaw"));
  });
  ...
  it("prefers OPENCLAW_HOME for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/blockclaw")).toBe(path.resolve("/srv/blockclaw-home", "blockclaw"));
