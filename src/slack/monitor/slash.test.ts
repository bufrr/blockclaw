...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "blockclaw_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "blockclaw_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("blockclaw_cmdarg")).toBe(true);
    expect(testHarness.options.has("blockclaw_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("blockclaw_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("blockclaw_cmdarg");
    expect(blockId).toContain("blockclaw_cmdarg_ext:");
    const token = (blockId ?? "").slice("blockclaw_cmdarg_ext:".length);
