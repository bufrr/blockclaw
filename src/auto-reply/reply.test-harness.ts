...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(home, ".blockclaw");
    process.env.OPENCLAW_AGENT_DIR = path.join(home, ".blockclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".blockclaw", "agent");
