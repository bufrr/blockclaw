...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "blockclaw-group-gating-"));
...
function makeInboundCfg(messagePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/blockclaw" } },
...
describe("applyGroupGating", () => {
  ...
  it("allows group messages when whatsapp groups default disables mention gating", () => {
    const cfg = makeConfig({
      ...
      messages: { groupChat: { mentionPatterns: ["@blockclaw"] } },
    });
