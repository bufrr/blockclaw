...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "blockclaw" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["blockclaw", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["blockclaw_service_marker"];
  const kindKeys = ["blockclaw_service_kind"];
...
function isOpenClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.blockclaw.");
}
...
function isOpenClawGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("blockclaw-gateway")) {
    return false;
  }
...
function isOpenClawGatewayTaskName(name: string): boolean {
  ...
  return normalized === defaultName || normalized.startsWith("blockclaw gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (marker === "blockclaw" && isOpenClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "blockclaw" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "blockclaw" && isOpenClawGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
