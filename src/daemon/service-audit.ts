...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENCLAW_GATEWAY_TOKEN does not match gateway.auth.token in blockclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `blockclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
