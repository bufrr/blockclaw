...
async function ensureOpenClawExtensions(manifest: PackageManifest) {
  ...
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing blockclaw.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json blockclaw.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  // Prefer the canonical `id` from blockclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-blockclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "blockclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "blockclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "blockclaw-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "blockclaw-npm-pack-",
    spec,
    timeoutMs,
