...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `blockclaw-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "blockclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("rejects manifest paths that escape plugin root via symlink", () => {
    ...
    const outsideManifest = path.join(outsideDir, "blockclaw.plugin.json");
    const linkedManifest = path.join(rootDir, "blockclaw.plugin.json");
